<template>
  ...
  <div class="course">
    ...
    <ul class="course-info" v-show="!login">
      ...
      <li class="one" @click = "upshow($event)" v-for="(info,index) in classtea_listw" :style="{top:(2.1+index*0.7)+'rem'}">
...
//              zIndex:'0',
    ...
    mounted(){
      ...
      Axios.get('http://localhost:3000/courseList_teacherw',{params:{
        ...
        studentstatew:this.statew

      }}).then((res)=>{
        this.classtea_listw = JSON.parse(res.data);
//        console.log(this.classtea_list.length);
//        this.$store.dispatch('sentlengthw',this.classtea_list.length);    //长度
